...
카프카는 이러한 특성을 이용해서 resilience 를 제공한다.
...
그럼 특정 파티션의 메시지가 소비되지 못한 메시지들의 수가 늘어나게 된다.

이러한 상황에서 카프카는 rebalance 라는 과정을 거치게 된다.
...
rebalance 를 통해 정상 상태의 컨슈머에게 해당 파티션의 점유권을 이전하게 되고, 다른 컨슈머가 이어서 처리를 할 수 있도록 한다
...
컨슈머 그룹 내에서 적절히 정상 상태의 컨슈머에 파티션을 할당시켜줘서 탄력성, resilience 를 제공한다

이 과정을 kafka 에서는 리밸런싱, rebalancing 이라고 부른다

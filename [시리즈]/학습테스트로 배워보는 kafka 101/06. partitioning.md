[학습 테스트로 배워보는 kafka] 7. 학습 테스트로 카프카 파티션(kafka partition) 알아보기
...
앞선 파티션 할당 동작중 해시 과정을 거친다고 했는데, 여기서 주의할 점이 있다.

아래의 테스트를 확인해보자

[##_Image|kage@bmEauN/btsrZlcCXoK/hxvSNJYle4FKAHojfOjVXk/img.png|CDM|1.3|{"originWidth":1476,"originHeight":552,"style":"alignCenter"}_##]

### 테스트 설명

- 주석 1
  - kafka 는 내부적으로 hash 를 하기 위해 [murmur hashing](https://en.wikipedia.org/wiki/MurmurHash) 를 한다.
  - key-1 이라는 문자열을 hash 하고 임의의 파티션 수(2) 만큼 mod 연산을 수행하여 **파티션 번호를 구한다**
- 주석 2.
  - key-2 이라는 문자열을 파티션 키로 설정해 **파티션 번호를 구한다**
- 주석 3.
  - **두 키가 달라야 한다는** 테스트 단언문이다

이 테스트는 서로 다른 키를 명시했기 때문에 실패해야한다.

[##_Image|kage@CTfuk/btsrZCrDqqY/k1mUK0gO5yZc05SFQmhnLK/img.png|CDM|1.3|{"originWidth":2364,"originHeight":560,"style":"alignCenter"}_##]

하지만 안타깝게도 실패한다.

이것을 **파티션 키 충돌**이라고 한다.

서로 다른 파티션 키를 가졌더라도 해시값을 파티션 수로 mod 연산한 결과가 동일하기 때문에 이와 같은 문제가 발생하는 것이다.

개발자는 파티션 키를 잘 할당했다고 생각하지만 실제로 파티션 키가 동일하기 떄문에 **한쪽으로 몰림 현상**이 발생할 수 있는데, 이로 인해서 파티셔닝을 했지만 **실질적으로는 아무런 효과도 얻을 수 없는 상황**이 되어버린다.

이런 키 충돌을 최소화 하기 위해서 키에 대해서 사전에 충분한 테스트 과정이 있거나 **파티션 수를 늘리는 것**을 고려해야할 수도 있다.
...
발행 순서대로 소비하지 않는것을 확인하는 위의 테스트 역시 통과한다.
...
하지만 파티션 수가 적다면 메시지 처리량이 줄어들거나 앞서 이야기한 것 처럼 파티션 키가 충돌되어 파티셔닝을 한 의미가 없어질 수 있다.

물론 상황과 조건에 따라서 결정 사항이 계속 바뀌게 될 것인데, 이러한 사항들을 인지하고 있으면 선택에 도움이 될 것이다

# 학습 테스트 정리

- ✅ 파티션 키를 명시하면 해당 키에 해당하는 파티션에 메시지(이벤트 로그)가 발행된다
- ✅ 파티션 키를 명시하지 않으면 partitioner 에 의해 적절히 파티션에 고르게 분배된다
- ✅ 파티션 키를 명시하더라도 파티션 할당 충돌이 발생할 수 있으므로 그 점을 고려해야한다
- ✅ 파티션 내에서는 순서를 보장한다
